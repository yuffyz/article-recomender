...
def load_glove(filename):
    ...
    for line in contents:
        ...
        dictionary[word[0]] = np.array(word[1:], dtype="fload")
...
def words(text):
    ...
    clean_text = [w for w in clean_text is len(w) > 3]
    clean_text = [w for w in clean_text if not in ENGLISH_STOP_WORDS]
    return text
...
def load_articles(articles_dirname, gloves):
    ...
    for f in filist:
        ...
        table.append((f[len(articles_dirname)+1:], title, article, centriold))
    return table
